// Assignment 1 - Poor Man's Piano
// A clumsy attempt to synthesize a piano sound and 4-voice polyphony using 8 oscillators
// In order to save my sanity, I had to organise this a bit using functions and pretty it up a bit, using envelopes and reverb (even though this 
// hasn't been covered in class - I read the manual)
...
// This function sets the times of all notes. Also sets the frequencies. Each sound can be muted (0) or played (1)
function void playNotes(dur duration, float freq1, int mute1, float freq2, int mute2, float freq3, int mute3, float freq4, int mute4)
{
    ...
    duration => now; //advance time
    
    //Sound off
    keysOff();
}

//Calculate the frequency to be played from the fundamental frequency and the given partial
function float getFrequency (float ff, float partial)
{
    return ff * Math.pow(2.0, partial/12);
}

// Initialise envelopes for all oscillators
...
// Set decay times for all oscillators. 70% of duration for sine waves, 3% of duration for square waves for that initial clicky sound
